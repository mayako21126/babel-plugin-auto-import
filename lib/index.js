 ...
 * @LastEditors: mayako
 * @LastEditTime: 2019-11-26 09:58:55
 */
function getUrl(obj) {
  let tmp = []
...
autoImport.prototype.apply = function (compiler) {
  ...
  compiler.hooks.compilation.tap('autoImport', function (compilation, options) {
    compilation.hooks.htmlWebpackPluginBeforeHtmlProcessing.tap('autoImport', function (htmlPluginData, callback) {
      ...
      htmlPluginData.html = htmlPluginData.html.replace(/<meta charset="utf-8">/, '<meta charset="utf-8"><meta name="autoModules" content=\'' + JSON.stringify(tmp) + '\'>')
      let tmp1 = setting.singleSpa ? 'if(!window.singleSpaNavigate){add(v)}else{add(assetsJs)}' : 'add(v)'
      let tmp2 = setting.singleSpa ? 'window.onload=function(){console.log("123");if(!window.singleSpaNavigate){window.' + name + '.mount()}}' : ''
